...
int __attribute__((optimize("O0"))) sensors_grab(struct IMUData *data, int samplecount, int sample_usec) {
  int res, ii;
  struct timespec ts;
  unsigned char buf[16];
  ...
  if(!i2c) { // check if i2c is initalized
    errno = EINVAL;
    return -1;
  }
  ...
  for ( ii = 0; ii < samplecount; ii++ ) {
    ...
    if(LPS25H_status & 2) { // pressure available
    ...
    if(HTS221_status & 2) { // humidity available
    ...
    if(HTS221_status & 1) { // temperature available
   ...
   *
   * Calculate pressure, temperature and humidity from median sample
   */
  res = 0;
  for ( ii = 0; ii < samplecount; ii++ ) {
